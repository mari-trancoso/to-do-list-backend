...
import { TTaskDB, TUserTaskDB } from './types'
...
app.post("/tasks", async (req: Request, res: Response) => {
    try {
        ...
        const [insertedTasks] = await db("tasks").where({id})
        res.status(201).send({
...
app.delete("/tasks/:id", async (req: Request, res: Response) => {
    try {
        ...
        const [taskIdToDelete] = await db("tasks").where({id:idToDelete})
...
//POST TASK BY ID add responsável pra uma tarefa
...
//DELETE uma pessoa responsável de uma tarefa
